...
`;
  ...
  private getAgentsContent(n8nVersion: string): string {
    return [
      ...
      `  - Example: \`this.Agent.uses({ ai_languageModel: this.Model.output, ai_tool: [this.Tool1.output, this.Tool2.output] })\``,
      `- ❌ Never use \`.out().to()\` for AI sub-node connections (ai_languageModel, ai_memory, ai_tool, ai_outputParser)`,
      ``,
      `---`,
      ``,
      `## 📚 Available Tools`,
