...
`;
  ...
  private getAgentsContent(n8nVersion: string): string {
    return [
      ...
      ``,
      `> **💡 Tip**: \`n8nac\` commands can be invoked three ways: \`./n8nac\` (local shim from VS Code extension), \`n8nac\` (global install / PATH), or \`npx n8nac\` (resolve from node_modules without global install). Examples below use the bare form \`n8nac\`.`,
      ``,
      ...
      `## 📚 Available Tools`,
      ``,
      `> **💡 Tip**: Three ways to invoke the CLIs depending on your setup:`,
      `> - \`./n8nac-skills\` / \`./n8nac\` — local shims generated by the VS Code extension (preferred in extension context)`,
      `> - \`n8nac-skills\` / \`n8nac\` — global install or PATH-based resolution`,
      `> - \`npx n8nac-skills\` / \`npx n8nac\` — resolve from npm without a global install (CLI installed in node_modules)`,
      `>`,
      `> Examples below use \`./n8nac-skills\`; substitute with \`n8nac-skills\` or \`npx n8nac-skills\` as appropriate.`,
      ``,
      ...
      ``,
      `**#1**: Use \`n8nac-skills\` tools to prevent hallucinations (\`./n8nac-skills\`, \`n8nac-skills\`, or \`npx n8nac-skills\`)`,
      `**#2**: Follow the exact schema - no assumptions, no guessing`,
      `**#3**: Create workflows that work on the first try`,
      ``,
      `**When in doubt**: \`n8nac-skills get <nodeName>\` (or \`./n8nac-skills get <nodeName>\` / \`npx n8nac-skills get <nodeName>\`)`
    ].join('\n');
  }
